...
static void bsp_InitKeyHard(void)
{
	...
	GPIO_InitStructure.GPIO_Mode =GPIO_Mode_Out_PP ;	/* 输出推免模式 */
...
static void bsp_InitKeyVar(void)
{
	...
	s_tBtn[KID_JOY_U].RepeatSpeed = 100;	/* 每隔1000ms自动发送键值 */

	s_tBtn[KID_JOY_D].LongTime = 100;
	s_tBtn[KID_JOY_D].RepeatSpeed = 100;	/* 每隔1000ms自动发送键值 */

	s_tBtn[KID_JOY_L].LongTime = 100;
	s_tBtn[KID_JOY_L].RepeatSpeed = 100;	/* 每隔1000ms自动发送键值 */

	s_tBtn[KID_JOY_R].LongTime = 100;
	s_tBtn[KID_JOY_R].RepeatSpeed = 100;	/* 每隔1000ms自动发送键值 */
	#endif
	/* 判断按键按下的函数 */
