...
{
	__HAL_RCC_GPIOB_CLK_ENABLE();	//打开GPIOB外设时钟
...
uint8 SHT30_RecvByte()
{
  ...
  SHT30_SDA0_H; 						//ê1?ü?ú2?é?à-,×?±??áè?êy?Y,
	...
	for (i=0; i<8; i++) 					//8????êy?÷
  {
	SHT30_SCL0_H;						//à-??ê±?ó??
		SHT30_Delay1us(5);
		dat |= SHT30_SDA0_DAT;				//?áêy?Y			   
	SHT30_SCL0_L;						//à-μíê±?ó??
	SHT30_Delay1us(5);						//?óê±
...
uint8 SHT30_Single_Measure(uint8 *vBuf)
{
	...
	vRval |= SHT30_SendByte(SHT30_SlaveAddress+0);					//μ??·D′
	if (!vRval) vRval |= SHT30_SendByte( (SHT30_SINGLE_H_MEASURE_DIS>>8)&0xFF );				//ê1?ü?????è2é?ˉ
	...
	{
		SHT30_Start();
		vRAck = SHT30_SendByte(SHT30_SlaveAddress+1);					//μ??·?á
	...
	for(i=0; i<6; i++)
	{
		vBuf[i] = SHT30_RecvByte(); 				//′?′￠êy?Y
		if (i == 0x06)
		{
		  
		  SHT30_SendACK(1); 						//×?oóò???êy?YDèòa??NOACK
		}
		else
		{		
		  SHT30_SendACK(0); 						//??ó|ACK
		}
	}
