...
//清除先验频点 AT+NCSEARFCN
const char*  AT_NCSEARFCN      = "AT+NCSEARFCN";
...
//定义全局缓存变量
#define NB_UART_RECE_BUF_MAX_LEN   RECE_BUF_MAX_LEN//RECE_BUF_MAX_LEN 1024
#define NB_UART_SEND_BUF_MAX_LEN    1024//512
...
typedef struct
{
  ...
  uint8_t      nb95_ICCID[20+1];//集成电路卡识别码的缩写，用来标识SIM卡。20位。
  int SignalPower;
  int ECL;
  int SNR;
  int CellID;
...
//==============================================================================
//NBiot 操作状态
 module_state      g_nb_state   = {PROCESS_NONE,0}; 
...
#endif
SUB_CMEE, //SUB_CFUN,
SUB_CIMI,SUB_CGSN,SUB_ICCID,SUB_CEREG,SUB_CSCON,
SUB_CGATT,SUB_CGATT_QUERY,SUB_NSMI,
SUB_NNMI,
SUB_NUESTATS,// 上报无线参数数据
...
static Bool cmd_next()
{
  ...
  if(g_nb_state.state == PROCESS_INIT)
  {
	...
	case SUB_NCONFIG:
	  {
		cmd_param_init(&g_at_cmd,AT_NCONFIG,"AUTOCONNECT,FALSE",CMD_SET);
	  }
	  break;
	case SUB_NCDP:
	  {
		cmd_param_init(&g_at_cmd,AT_NCDP,REMOTE_COAP_INFO,CMD_SET);
	  }
	  break;
	case SUB_NRB:
	  {
		...
		g_at_cmd.cmd_try = 1;
        g_at_cmd.max_timeout = 10000;  //相邻指令之间限制3s
        g_at_cmd.lmt_period = TRUE;
        g_at_cmd.pExpectRes = "Neul";     //设置期望回复消息，如果指令执行完成
                                             //没有与期望的消息匹配，则认为出错
                                             //并进行出错尝试		
		#else
        cmd_param_init(&g_at_cmd,AT_NRB,NULL,CMD_READ);

		
		g_at_cmd.cmd_try = CMD_TRY_TIMES;
        g_at_cmd.max_timeout = 10000;  //相邻指令之间限制3s
        g_at_cmd.lmt_period = TRUE;
        g_at_cmd.pExpectRes = "REBOOT";     //设置期望回复消息，如果指令执行完成
                                             //没有与期望的消息匹配，则认为出错
                                             //并进行出错尝试
		#endif
	  }
	...
	case SUB_CFUN:
	  {
		/*模块重启之后，需要复位串口*/
		HAL_UARTDMA_Init(NB,bc95_receCb,9600);
		p_info("SUB_NRB");
	...
	case SUB_CEDRXS:
	  {
		cmd_param_init(&g_at_cmd,AT_CEDRXS,"0,5",CMD_SET);
	  }
	  break;
	case SUB_CPSMS:
	  {
		cmd_param_init(&g_at_cmd,AT_CPSMS,"0",CMD_SET);
	  }
	  break;
#endif
    ...
    case SUB_ICCID:
      {
	  	#if 0
        cmd_param_init(&g_at_cmd,AT_CICCID,"1",CMD_SET);
		#else
        cmd_param_init(&g_at_cmd,AT_CICCID,NULL,CMD_READ);
		#endif
      }
      break;	
    case SUB_CEREG:
    ...
    case SUB_CGATT:
      {
        cmd_param_init(&g_at_cmd,AT_CGATT,"1",CMD_SET);//触发网络附着
    ...
    case SUB_CGATT_QUERY:
      {
		...
		g_at_cmd.cmd_try = 60;//300s后，AT+CGATT?查NO 询返回值不等于1
        g_at_cmd.max_timeout = 5000;  //相邻指令之间限制3s
        g_at_cmd.lmt_period = TRUE;
    ...
    case SUB_NUESTATS:
      {
        //设置coap接收消息，给予指示
        cmd_param_init(&g_at_cmd,AT_NUESTATS,NULL,CMD_EXCUTE);
		g_at_cmd.cmd_try = 1;
      }
      break;
    }
...
uint8_t bc95_AsyncNotification(char* buf, uint16_t* len)
{
  ...
  if(position_addr_start = strstr(buf,"+NSONMI"))//指示已到达的 Socket 消息
  {
    //收到服务器端发来的UDP数据
    char* pColon = strchr(position_addr_start,':');
    if(pColon)
  ...
  else if(position_addr_start = strstr(buf,"+NNMI"))//  有消息接收到
  {
    isAsync = addr_adjust(buf,position_addr_start,len);
    //isAsync =TRUE;
    nbset_event(NB_COAP_RE_EVENT);  
	//bsp_StartTimer(TMR_ID_read_NB_buffer, TMR_DELAY_60S);// 主动读取NB模块缓存	
	//bsp_DelayMS(100);//100ms
  }
...
//
// return : none
extern int app_4G_msgReportCb(char* msg,uint16 len);
...
int bc95_open(NB_Handle handle)
{
  ...
  hw_handle->uart_fxnTable->openFxn(app_4G_msgReportCb, hw_handle->baudrate);
...
// fn : bc95_getICCID
//
// brief : 集成电路卡识别码的缩写，用来标识SIM卡。20位
//
// param : handle ->nb对象指针
//
// return : none
const char* bc95_getICCID(NB_Handle handle)
{
  return (const char*)g_bc95_status.nb95_ICCID;
}
...
//
// return : none
const char* bc95_getIMEI(NB_Handle handle)
{
  return (const char*)g_bc95_status.nb95_IMEI;
}
//******************************************************************************
// fn : bc95_getSignalPower
//
// brief : 读取模块
//
// param : handle ->nb对象指针
//
// return : none
int bc95_getSignalPower(void)
{
  return g_bc95_status.SignalPower;
}
//******************************************************************************
// fn : bc95_getSNR
//
...
//
// return : none
int bc95_getSNR(void)
{
  return g_bc95_status.SNR;
}
//******************************************************************************
// fn : bc95_getECL
//
...
//
// return : none
int bc95_getECL(void)
{
  return g_bc95_status.ECL;
}
//******************************************************************************
// fn : bc95_getCellID
//
...
//
// return : none
int bc95_getCellID(void)
{
  return g_bc95_status.CellID;
}
...
int bc95_coapSendMsg(NB_Handle handle,int len,char*msg)
{
  ...
  g_at_cmd.max_timeout = 3000;//、、5s
  g_at_cmd.cmd_try = 1;//发送重试1
...
void nbsend_msg_app(NB_Handle handle, char**buf,Bool isOk)
{
  ...
  if(g_nb_state.state == PROCESS_INIT)
  {
   ...
   //   break;
    ...
    case SUB_ICCID:
      {
        ...
        {
          pColon++;
          memcpy(g_bc95_status.nb95_ICCID,pColon,20);
          g_bc95_status.nb95_ICCID[20] = 0;
          handle->AppReceCB((msg_types_t)TYPES_ICCID,20,(char*)g_bc95_status.nb95_ICCID);
        }
      }
      break;	
    case SUB_NUESTATS:
    {
		#if 1
        char* pColon = strchr(buf[0],':');//Signal power:-722
        if(pColon)
        {
			pColon++;        
		    sscanf(pColon,"%d",&g_bc95_status.SignalPower);		
			g_bc95_status.SignalPower = g_bc95_status.SignalPower/10;//RSRP=Signal power/10 = 除以10
        }		
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
			if(*pColon==0x0a)// 寻找 0x00 			0x0a
				break;
		}		
		
        pColon = strchr(pColon,':');//Total power:-618
		pColon++;		 
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
		...
		pColon = strchr(pColon,':');//TX power:150
		pColon++;
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
        ...
        pColon = strchr(pColon,':');//TX time:1308
		pColon++;		 
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
		...
		pColon = strchr(pColon,':');//RX time:26691
		pColon++;
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
        ...
        pColon = strchr(pColon,':');//Cell ID:28937809
        if(pColon)
        {
			pColon++;
		    sscanf(pColon,"%d",&g_bc95_status.CellID);		
        }
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
        ...
        pColon = strchr(pColon,':');//ECL:1
        if(pColon)
        {
			pColon++;
		    sscanf(pColon,"%d",&g_bc95_status.ECL);		
        }
		for(uint8_t i = 0;i<65;i++)
		{			
			pColon++;        
        ...
        pColon = strchr(pColon,':');//SNR:-17
        if(pColon)
        {
			pColon++;
		    sscanf(pColon,"%d",&g_bc95_status.SNR);		
			g_bc95_status.SNR = g_bc95_status.SNR/10;//实际SNR*10  =   除以10
			
            handle->AppReceCB((msg_types_t)TYPES_NUESTATS,1,"S");
        }
		#endif
    }
      break;	  
  ...
  else if(g_nb_state.state == PROCESS_SIGN)
  {
    ...
    {
      char* pColon = strstr(buf[0],"+CSQ:");
	  if(pColon!=NULL)
 	 	{
		  pColon=pColon+strlen("+CSQ:");
  ...
  else if(g_nb_state.state == PROCESS_COAP_RE)
  {
    if(g_nb_state.sub_state == 1)
    {
      ...
      char* tmp_buf = NULL;
		...
		for(i=0;i<=6;i++)
		{
			 ...
			 if(tmp_buf)
			 {
			   tmp_buf++;
			   index =	NB_HexStrToNum(tmp_buf);
   			   //p_info("PROCESS_COAP_RE,%d:.%c",index,tmp_buf);
			...
			{
			  // p_info("PROCESS_COAP_RE 无 %d",i);//、、error
			}
		}
      ...
      if(index != 6)
      {
        handle->AppReceCB((msg_types_t)PROCESS_COAP_RE,1,"F");
        return;
      }
